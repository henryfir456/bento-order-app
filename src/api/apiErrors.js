...
const businessMessages = Object.freeze({
  ...
  EMPLOYEE_ALREADY_LINE_BOUND: '此員工編號已有 LINE ownership，已改用既有員工帳號登入；不會覆蓋原綁定。',
  LINE_LOGIN_REQUIRED: '此員工帳號可使用 LINE 或員工編號登入。',
  UNVERIFIED_EMPLOYEE: '基本資料完成後即可使用一般功能；LINE 綁定是選用功能。',
  ...
  ADMIN_LINE_AUTH_REQUIRED: '員編綁定需要已綁定員編的 LINE Admin 身份。',
  EMPLOYEE_BIND_CONFLICT: '員編綁定狀態已變更，請重新整理後再試。',
  TRUSTED_UNIQUE_ACTIVE: '可信員工資料已唯一匹配。',
  PENDING_TRUST_REVIEW: '此核驗欄位為歷史相容資訊，現行存取依 LINE 綁定與角色判定。'
});
