...
test('CHANGELOG.md preserves the complete pre-migration release history', async () => {
  ...
  const historical = parsed.filter((release) => (
    isFormalRelease(release) && !['0.10.0', '0.10.1', '0.11.0', '0.11.1', '0.11.2', '0.11.3', '0.12.0', '0.12.1', '0.13.0', '0.14.0', '0.14.1', '0.14.2'].includes(release.version)
  ));
  ...
  assert.deepEqual(uiReleases.map((release) => release.version).slice(0, 5), ['0.14.2', '0.14.1', '0.14.0', '0.13.0', '0.12.1']);
