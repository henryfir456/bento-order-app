...
const sessionRow = async (database, token) => {
  ...
  return database.prepare(`
    SELECT egs.session_id, egs.token_hash, egs.user_id, egs.auth_mode,
           ...
           u.balance, u.role, u.active, u.created_at AS user_created_at,
           u.updated_at AS user_updated_at
    FROM employee_guest_sessions egs
    JOIN users u ON u.user_id = egs.user_id
...
export const createGuestSession = async (
  database,
  { userId, clock = new Date(), lifetimeMs = DEFAULT_LIFETIME_MS } = {}
) => {
  ...
  await database.prepare(`
    INSERT INTO employee_guest_sessions (
      session_id, token_hash, user_id, auth_mode, created_at, expires_at
